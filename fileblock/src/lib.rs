...
impl BlockStorage for FileBlockEmulator {
    /// Allocates a new file of the correct size by zeroing out the entire file block.
    /// Disks aren't necessarily zero'd out in the same way but doing so in the file ensures
    /// that the file is allocated to a fixed size.
    ...
    fn write_block(&mut self, blocknr: BlockNumber, mut buf: &mut [u8]) -> std::io::Result<()> {
        if blocknr > (self.block_count - 1) {
            return Err(std::io::Error::new(
                ErrorKind::InvalidInput,
                "block requested exceeds filesystem upper bound",
